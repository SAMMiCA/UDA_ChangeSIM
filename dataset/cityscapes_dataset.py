...
class CityScapesDataSet(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        noise = np.random.choice(2, size=(image.shape[0], image.shape[1], image.shape[2]), p=[0.7, 0.3])
        ...
        label = image.astype(np.float32)

        image_noise = image_noise[:, :, ::-1]  # change to BGR
        image_noise = image_noise / 255.0
        image_noise = image_noise.transpose((2, 0, 1))
