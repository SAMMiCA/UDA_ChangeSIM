...
class SegHelper:
    ...
    def classmap2colormap(self,seg_map_class):
        ...
        seg_map_color_flattened = torch.zeros(seg_map_class.shape[0]*seg_map_class.shape[1],3).long()
        for cls, color in self.idx2color.items():
            matching_indices = (seg_map_class_flattened == torch.LongTensor([cls]))
            seg_map_color_flattened[matching_indices.view(-1)] = torch.LongTensor(color)
