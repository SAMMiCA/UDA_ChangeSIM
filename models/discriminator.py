...
class DHA(nn.Module):
	def __init__(self, discriminator, relative=False):
	...
	def forward(self, target, source, loss_type):
		...
		if loss_type == 'adversarial':
			if self.relative:
				...
				source_relative = source_feat - torch.mean(target_feat, 0, keepdim=True)
				target_relative = target_feat - torch.mean(source_feat, 0, keepdim=True)

				loss = (torch.mean(torch.nn.ReLU()(1 + source_relative)) +
						torch.mean(torch.nn.ReLU()(1 - target_relative)))
