...
class ResNet_multi(nn.Module):
    def __init__(self, block, layers, args=None):
        ...
        if args.use_scale:
            scale1 = torch.randn(args.batch_size, 1024, 30, 40).fill_(20)
            scale2 = torch.randn(args.batch_size, 2048, 30, 40).fill_(20)

            scale1 = scale1 * self.k_init
            scale2 = scale2 * self.k_init

            self.scale1 = nn.Parameter(scale1.cuda(), requires_grad=True)
            self.scale2 = nn.Parameter(scale2.cuda(), requires_grad=True)
    ...
    def forward(self, image, input_size):
        pred2_total, pred1_total, pred2_tm, pred1_tm = None, None, None, None
        ...
        if self.tm:
            if self.use_scale:
                x3_1, x3_2 = getattr(self, TM_name1)(x * self.scale1)
            else:
                x3_1, x3_2 = getattr(self, TM_name1)(x)

            new_x = x1 + x3_1 + x3_2
            pred1_total = nn.Upsample(size=(input_size[1], input_size[0]), mode='bilinear', align_corners=True)(new_x)  # ResNet + (ASPP1+TM1)

            new_x_tm = x3_1 + x3_2
            pred1_tm = nn.Upsample(size=(input_size[1], input_size[0]), mode='bilinear', align_corners=True)(new_x_tm)  # ResNet + (TM1)
        ...
        if self.tm:
            if self.use_scale:
                x3_1, x3_2 = getattr(self, TM_name2)(x * self.scale2)
            else:
                x3_1, x3_2 = getattr(self, TM_name2)(x)

            new_x = x2 + x3_1 + x3_2
            pred2_total = nn.Upsample(size=(input_size[1], input_size[0]), mode='bilinear', align_corners=True)(new_x)  # ResNet + (ASPP2+TM2)

            new_x_tm = x3_1 + x3_2
            pred2_tm = nn.Upsample(size=(input_size[1], input_size[0]), mode='bilinear', align_corners=True)(
                new_x_tm)  # ResNet + (TM2)

        return pred2_total, pred1_total, pred_ori2, pred_ori1, pred2_tm, pred1_tm
